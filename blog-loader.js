fetch('blog-data.json')
  ...
  .then(posts => {
    ...
    posts.reverse().slice(0, 5).forEach(post => {
      ...
      card.style.cssText = 'text-align: left; cursor: pointer; min-height: 100%; justify-content: space-between; display: flex; flex-direction: column;';
      ...
      card.innerHTML = `
        ...
        <a href="${post.slug}" style="display: inline-block; padding: 0.5rem 1.1rem; background-color: #00d4ff; color: #0f2027; border-radius: 6px; text-decoration: none; font-weight: bold; font-size: 0.85rem; text-align: center;">
          Read More →
        </a>
      `;
